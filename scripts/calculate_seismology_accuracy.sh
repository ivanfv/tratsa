...
for i in "${benchs[@]}"; do
  for e in $exp; do
    for m in $man; do
              for t in $til; do
      		echo "## $i $e $m"
		./accuracy/check_accuracy references/seismology/MP_seismology_scaled_w64_t${t}_double.txt references/seismology/MP_seismology_scaled_w64_t${t}_float.txt outputs/seismology/MP_seismology_scaled.txt_64_${e}_${m}_${t}.txt ./accuracy_results/seismology/accuracy_seismology_w64_e${e}_m${m}_t${t}.txt &
	done;
    done;
  done;
done;
