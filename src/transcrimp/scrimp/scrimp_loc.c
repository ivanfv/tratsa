...
void scrimp_loc(double * tSeries, double * AMean, double * ASigma,
        int timeSeriesLength, int ProfileLength,
        int windowSize, int* idx, double * profile, int * profileIndex,
        int exclusionZone, int numThreads, int diagsPerTh) {
  ...
  //RIC compruebo que el número de diagonales sea divisible entre diagsPerTh
  printf("Num diags: %d\n", ProfileLength - (exclusionZone + 1));
  if( ((ProfileLength - (exclusionZone + 1)) % diagsPerTh) != 0)
    assert("Diagonales no divisibles entre diagsPerTh" && 0);
  
#pragma omp parallel
  {
    double distance, windowSizeDTYPE, *lastz;
    int diag, my_offset, i, j, k, ri;
    
    lastz = (double *) malloc(diagsPerTh*sizeof(double));
    windowSizeDTYPE = (double) windowSize;
    my_offset = omp_get_thread_num() * ProfileLength;

#pragma omp for schedule(dynamic)
    for (ri = 0; ri < (ProfileLength - (exclusionZone + 1)); ri+=diagsPerTh) {
      diag = idx[ri];
      for(k = 0; k < diagsPerTh; k++)
        lastz[k] = 0;
      //RIC
      //printf("tid: %d. diag: %d\n", omp_get_thread_num(), diag);

      /* Dot product calculation -------------------------- */
      for (j = diag; j < windowSize + diag; j++) {
        //RIC calculo el producto escalar de la ventana en la diagonal con la
        //primera ventana de la fila (lo que ya estaba) y además calculo el
        //producto escalar de la ventana que está a un elemento a la derecha de
        //la diagonal
        for(k = 0; k < diagsPerTh; k++) {
          lastz[k] += tSeries[j+k] * tSeries[j - diag];
        }
      }
      ...
      /* Distance calculation ----------------------------- */
        distance = 2 * (windowSizeDTYPE - (lastz[k] - windowSizeDTYPE * AMean[j+k] * AMean[i]) / (ASigma[j+k] * ASigma[i]));
      /* -------------------------------------------------- */

      /* Profile update ----------------------------------- */
        if (distance < profile_tmp[my_offset + j + k]) {
          profile_tmp[my_offset + j + k] = distance;
          profileIndex_tmp [my_offset + j + k] = i;
          //printf("out tid= %d, prof[%d]\n", omp_get_thread_num(), j + k);
        }
        if (distance < profile_tmp[my_offset + i]) {
          profile_tmp[my_offset + i] = distance;
          profileIndex_tmp [my_offset + i] = j + k;
          //printf("out tid= %d, prof[%d]\n", omp_get_thread_num(), i);
        }
      } 
      /* -------------------------------------------------- */
      i = 1;
      //printf("ri = %d ", ri); fflush(stdout);
      //printf("ProfileLength: %d\n",ProfileLength);
      for (j = diag + 1; j < ProfileLength; j++) {
        //printf("ProfileLength - j: %d MIN: %d\n", ProfileLength - j, MIN(diagsPerTh, ProfileLength - j));
        for(k = 0; k < MIN(diagsPerTh, ProfileLength - j); k++) {
          /* Dot product update ----------------------- */
          lastz[k] += (tSeries[j + k + windowSize - 1] * tSeries[i + windowSize - 1]) - (tSeries[j + k - 1] * tSeries[i - 1]);
          /* ------------------------------------------ */

          /* Distance calculation --------------------- */
          distance = 2 * (windowSizeDTYPE - (lastz[k] - AMean[j+k] * AMean[i] * windowSizeDTYPE) / (ASigma[j+k] * ASigma[i]));
          /* ------------------------------------------ */
...
int main(int argc, char* argv[]) {
  ...
  int timeSeriesLength, numThreads, diagsPerTh;
  ...
  if (argc != 6) {
    printf("[ERROR] usage: ./scrimp timeseries.txt window_size"
            " num_threads diags_per_th scale_factor\n");
    return 0;
  }
  windowSize = atoi(argv[2]);
  numThreads = atoi(argv[3]);
  diagsPerTh = atoi(argv[4]);
  ...
  for (int i = 0; i < timeSeriesLength; i++) {
    ...
    ff_init_double(&tSeries_ff[i], tSeries[i], (flexfloat_desc_t) {dotp_exp, dotp_man});
  }
  fclose(fp);
  printf("[INFO] DONE\n");
  /* ------------------------------------------------------------------ */
  ...
  for (int i = 0; i < ProfileLength; i++) {
    ff_init_double(&AMean_ff[i], AMean[i], (flexfloat_desc_t) {stats_exp, stats_man});
    ff_init_double(&ASigma_ff[i], ASigma[i], (flexfloat_desc_t) {stats_exp, stats_man});
    ff_init_double(&profile_ff[i], profile[i], (flexfloat_desc_t) {prof_exp, prof_man});
  }

  flexfloat_t windowSize_ff;

  ff_init_double(&windowSize_ff, windowSize, (flexfloat_desc_t) {dist_exp, dist_man});
  /* ------------------------------------------------------------------ */
  ...
  scrimp_loc(tSeries, AMean, ASigma, timeSeriesLength, ProfileLength,
          windowSize, idx, profile, profileIdxs, exclusionZone,
          numThreads, diagsPerTh);
  ...
  sprintf(path_result, "%s%s_w%d_s%.2f_t%d_d%d_%d.csv", PATH_RESULT,argv[1], windowSize, scaleFactor, numThreads, diagsPerTh, getpid());
